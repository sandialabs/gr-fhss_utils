...
from gnuradio.fhss_utils import RMS, HALF_POWER, COERCE
...
class fsk_burst_extractor_hier(gr.hier_block2):
    ...
    block structure is:
         ...
         |-------|   |--------|   |--------|   |--------|   |---------|
         |  FFT  |   | Tagged |   |   CF   |   |  FIR   |   |  Fine   |
    In-->| Burst |-->|Burst To|-->|estimate|-->| Filter |-->|  Time   |--> Out
         | Tagger|   |  PDU   |   |        |   |        |   | Measure |
         |-------|   |--------|   |--------|   |--------|   |---------|
    ...
    def __init__(self, burst_width=int(500e3), center_freq=915e6, decimation=32,
      ...
      '''
        ...
        self.fhss_utils_tagged_burst_to_pdu_0 = fhss_utils.tagged_burst_to_pdu(
            decimation, taps, min_burst_time, max_burst_time, 0.0, 1.0, 1.0, samp_rate, n_threads)
        self.fhss_utils_fft_burst_tagger_0 = fhss_utils.fft_burst_tagger(center_freq, fft_size, samp_rate, int(round((float(samp_rate) / fft_size) * pre_burst_time)), int(round((float(
            samp_rate) / fft_size) * post_burst_time)), burst_width, 0, 0, threshold, int(round((float(samp_rate) / fft_size) * hist_time)), int(round((float(samp_rate) / fft_size) * lookahead_time)), False)
    ...
    def set_threshold(self, threshold):
        self.fhss_utils_fft_burst_tagger_0.set_threshold(threshold)
